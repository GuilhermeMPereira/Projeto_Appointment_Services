...
const RenderBusca = ({ user }) => {
  ...
  const solicitarAgendamento = async (hora) => {
    try {
      ...
      // Verifica plataforma para alert
      if (Platform.OS === 'web') {
        window.alert("Sucesso: Solicitação enviada! O prestador analisará seu pedido.");
      } else {
    ...
    >
      <View style={clienteStyles.overlay}>
        ...
        <View style={clienteStyles.content}>
          {view === 'Busca' && <RenderBusca user={user} />}
          {view === 'Pedidos' && <RenderMeusPedidos user={user} />}
