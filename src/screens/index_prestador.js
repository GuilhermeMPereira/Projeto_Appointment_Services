...
import { useNavigation } from '@react-navigation/native'; // <--- 1. Importação necessária
...
export default function PrestadorHomeScreen() {
  ...
  const navigation = useNavigation(); // <--- 2. Inicializa a navegação
          ...
          renderItem={({item}) => (
            <View style={[styles.card, item.status === 'confirmado' && {borderLeftColor: 'green', borderLeftWidth: 5}]}>
              <View style={{flexDirection:'row', justifyContent:'space-between', alignItems:'center'}}>
                ...
                <TouchableOpacity 
                   onPress={() => navigation.navigate('Chat', { chatId: item.id, title: item.clienteNome })}
                   style={{padding:5}}
                >
  ...
  const RenderAgendaVisual = () => {
    ...
    return (
      <ScrollView>
        ...
        <Calendar 
          markedDates={{...marcados, [diaSelecionado]: {selected: true, selectedColor: '#0056B3'}}} 
          onDayPress={(day) => verDetalhesDia(day.dateString)} 
        />
        
        {diaSelecionado && (
          <View style={{marginTop: 20}}>
            <Text style={{fontWeight:'bold', marginBottom:10, fontSize:16}}>Serviços em {diaSelecionado}:</Text>
            
            {listaDia.length === 0 ? <Text style={{fontStyle:'italic', color:'#777'}}>Nada agendado para este dia.</Text> : null}

            {listaDia.map(item => (
              <View key={item.id} style={styles.itemAgenda}>
                <View style={{flex:1}}>
                   <Text style={{fontWeight:'bold', color:'#0056B3'}}>{item.horaString}</Text>
                   <Text>{item.clienteNome} - {item.servicoNome}</Text>
                </View>
                
                {/* --- BOTÃO DE CHAT NA AGENDA --- */}
                <TouchableOpacity 
...
const styles = StyleSheet.create({
  ...
  // ESTILO AGENDA
  itemAgenda: { flexDirection: 'row', backgroundColor: '#E9ECEF', padding: 10, borderRadius: 5, marginBottom: 5, alignItems: 'center', justifyContent:'space-between' },
  btnChatSmall: { backgroundColor: '#007bff', paddingHorizontal: 10, paddingVertical: 5, borderRadius: 5 },
